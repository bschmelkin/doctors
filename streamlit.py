...
def get_mongo_client():
    try:
        client = MongoClient(
            f"mongodb+srv://{st.secrets['mongo']['username']}:{st.secrets['mongo']['password']}@{st.secrets['mongo']['host']}/?retryWrites=true&w=majority",
            serverSelectionTimeoutMS=5000
        )
        db = client['doctors']
        offices_collection = db['offices']
        st.success("Connected to MongoDB successfully.")
        return client, offices_collection
    except Exception as e:
        st.error(f"An error occurred: {e}")
        return None, None

client, offices_collection = get_mongo_client()

if offices_collection is not None:
